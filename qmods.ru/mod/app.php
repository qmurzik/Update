...
if ($share === '' || !is_array($linkData) || empty($linkData['enabled']) || empty($linkData['token']) || !hash_equals((string)$linkData['token'], $share)) {
    ...
    ?><!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Ссылка недействительна — QMODS</title><style>body{margin:0;font-family:Inter,system-ui,Arial,sans-serif;color:#eceffd;display:grid;place-items:center;min-height:100vh;background:radial-gradient(800px 600px at 50% -10%,rgba(168,85,255,.3),transparent 62%),linear-gradient(180deg,#04040c,#08061c 55%,#04040c)}.box{max-width:460px;margin:24px;padding:34px 30px;background:linear-gradient(158deg,rgba(255,255,255,.075),rgba(255,255,255,.02));border:1px solid rgba(255,255,255,.12);border-radius:30px 8px 30px 8px;text-align:center;box-shadow:0 30px 80px rgba(2,2,10,.7)}.k{width:180px;height:auto;margin:-6px auto 6px;display:block;filter:drop-shadow(0 18px 28px rgba(0,0,0,.6)) drop-shadow(0 0 26px rgba(168,85,255,.35));-webkit-mask-image:linear-gradient(180deg,#000 78%,transparent 100%);mask-image:linear-gradient(180deg,#000 78%,transparent 100%)}h1{margin:0 0 10px;font-size:26px;letter-spacing:-.04em}p{color:#98a1c6;line-height:1.6;font-size:14.5px}a{display:inline-block;margin-top:14px;padding:14px 22px;border-radius:14px;background:linear-gradient(135deg,#6a24d6,#a855ff 48%,#ff3ec8);color:#fff;text-decoration:none;font-weight:700;box-shadow:0 16px 40px rgba(168,85,255,.36)}</style></head><body><div class="box"><img src="assets/kira/notfound.webp" alt="Kira" width="272" height="321" class="k"><h1>Ссылка недействительна</h1><p>Публичная ссылка на приложение была отключена или заменена новой.</p><a href="index.php">Перейти на QMODS</a></div></body></html><?php
    exit;
}
