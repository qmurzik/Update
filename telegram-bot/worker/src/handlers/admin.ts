...
import { adminMenuKeyboard, adminUserCardKeyboard, cancelKeyboard, confirmKeyboard } from '../telegram/keyboards';
import { esc, splitTitleBody } from '../util';
import { isAdmin } from '../config';
import { clearState, getState, logAdminAction, setState } from '../db';
import type { AdminUserCard } from '../qmodsApi';
...
export async function showAdminMenu(ctx: Ctx): Promise<void> {
  ...
  await reply(ctx, '<b>🛠 Админ-панель QMods</b>', adminMenuKeyboard());
}
...
export async function askIssue(ctx: Ctx): Promise<void> {
  ...
  await reply(ctx, `На сколько дней продлить подписку для <b>${esc(username)}</b>? Введите число.`, cancelKeyboard('adm:menu'));
}
...
export async function handleIssueInput(ctx: Ctx, username: string, daysText: string): Promise<void> {
  ...
  if (!Number.isFinite(days) || days < 1 || days > 3650) {
    await reply(ctx, 'Введите целое число дней от 1 до 3650.', cancelKeyboard('adm:menu'));
    return;
  }
  ...
  if (!res.success) {
    await reply(ctx, `❌ ${esc(String(res.error ?? 'Ошибка'))}`, cancelKeyboard('adm:menu'));
    return;
  }
  await reply(ctx, `✅ ${esc(String(res.message ?? 'Готово'))}`, cancelKeyboard('adm:menu'));
}

export async function askRemove(ctx: Ctx): Promise<void> {
  const username = await currentCardUsername(ctx);
  if (!username) return showAdminMenu(ctx);
  await reply(ctx, `Снять подписку с <b>${esc(username)}</b>?`, confirmKeyboard('adm:rm:yes', 'adm:menu'));
}
...
export async function askDelete(ctx: Ctx): Promise<void> {
  const username = await currentCardUsername(ctx);
  if (!username) return showAdminMenu(ctx);
  await reply(ctx, `⚠️ Полностью удалить аккаунт <b>${esc(username)}</b>? Действие необратимо.`, confirmKeyboard('adm:del:yes', 'adm:menu'));
}
...
export async function handleMessageInput(ctx: Ctx, username: string, text: string): Promise<void> {
  ...
  await reply(ctx, res.success ? '✅ Сообщение отправлено.' : `❌ ${esc(String(res.error ?? ''))}`, cancelKeyboard('adm:menu'));
}
