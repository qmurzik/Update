...
## Проверка подписки — только при запуске

`SubscriptionCheckRunnable` запускается один раз, в `onCreate` (холодный
старт), плюс сразу после `SubscriptionChecker.unlink()` (долгий «Назад» /
App Shortcut «Выйти» — см. ниже), чтобы сразу отправить на экран входа.
Периодического фонового тика **больше нет** — раньше `onResume()`
планировал повторную проверку каждые ~90 секунд через
`Handler.postDelayed`, пока `MainActivity` в foreground, но это оказалось
хуже, чем просто отсутствие live-проверки: любое короткое пропадание сети
во время обычного использования (лифт, метро, смена Wi-Fi/LTE) выкидывало
пользователя на полноэкранный «Проверьте интернет» посреди работы с
приложением — сначала пробовали пропускать такие транзитные ошибки
(`subscriptionEverActive:Z`), но раз самого тика больше нет, надобность в
этом тоже отпала, и оба убрали вместе.

**Следствие**: отзыв устройства, окончание подписки, принудительное
обновление версии и новые app-уведомления теперь применяются только при
следующем **перезапуске** приложения, а не мгновенно в уже открытом —
это сознательный компромисс в пользу того, чтобы связь никогда не
прерывала уже работающий сеанс.

Ответ `/device/subscription` (расширенный, см. `API.md`) при холодном
старте несёт сразу три вещи:

1. **`subscription`** — `{active, days_left, plan}`.
2. **`notifications`** — уведомления от админа (см. ниже), показываются как
   системные (через `AppNotifier`).
3. **`force_update`** — `{required, message}`; при `required: true` вызывается
   ...
   (кнопка ведёт на `https://qmods.ru/mod/download.php`).
...
**Один аккаунт — одно устройство (или два, если куплен «клон»).** Лимит по
умолчанию — 1; покупка «клона» за 200 ₽ (кнопка «⚙️ Устройства» в
боте/Mini App, см. README.md «Лимит устройств и „клон“») навсегда поднимает
его до 2, независимо от продления самой подписки — только пока подписка
активна, слот вообще что-то значит (см. `device_subscription` ниже).
Попытка привязки сверх лимита **отклоняется** сервером (не переписывает
старый токен молча) — pairing-запись помечается `status: "rejected",
reason: "device_limit"`, приложение видит это сразу через
`/device/pair/status` (а не ждёт 5-минутный таймаут), и `GateActivity`
показывает понятное сообщение: «У вас уже привязано максимум устройств.
Отвяжите одно в разделе «Устройства» бота…». Чтобы привязать новое
устройство сверх лимита, старое нужно сначала отвязать через бота/Mini App.
...
X5 — отдельное, отдельно продаваемое приложение (см. README.md, раздел
"X5"): своя подписка, своё устройство, свои (более высокие) цены в боте —
но живёт на том же аккаунте qmods.ru и на том же самом сервере, что и
основное приложение. Авторизация X5 полностью отделена на бэкенде: своя
запись `device_token` в D1 (с полем `app = 'x5'`), свой лимит устройств,
своя проверка подписки — привязка/оплата/отвязка одного приложения никак
не пересекается с другим, даже если оба стоят на одном телефоне одного и
того же пользователя.

**Это тот же самый smali-модуль — отдельной папки под X5 в репозитории
нет**, и заводить её не нужно: поддерживать два почти идентичных набора
`.smali`-файлов вручную — верный способ рано или поздно рассинхронизировать
их. Вместо этого сборка X5 отличается от обычной сборки (см. «Сборка в
APK» выше) ровно одной строкой:

1. Пройдите шаги 1–6 «Сборка в APK» как обычно, декомпилировав уже ВТОРОЙ
   APK (тот, что пойдёт под X5 — свой пакет, свой App ID, как для любого
   отдельного приложения).
2. **Единственное отличие** — в `DevicePairingRunnable.smali`, метод
   `run()`, в самом начале (см. комментарий там же, помеченный "X5
   BUILD"): к URL привязки допишите `?app=x5`:
   ```smali
   const-string v1, "https://update.qmurzik7.workers.dev/device/pair/start?app=x5"
   ```
   вместо обычного (без `?app=x5` — это и есть значение по умолчанию,
   `main`). Больше нигде ничего менять не нужно — `SubscriptionCheckRunnable`
   трогать не надо вообще: Worker сам узнаёт, какому приложению принадлежит
   `device_token`, из своей же D1-таблицы (записано туда один раз, ещё на
   шаге привязки), а не из того, что пришлёт клиент — так безопаснее и
   проще.
3. Проверено реальным ассемблером (см. «Как это проверялось» в начале
   файла) — вариант с `?app=x5` собирается в валидный DEX без ошибок,
   тем же способом, что и весь остальной модуль.

Поскольку это в итоге два разных APK (разные пакеты, разные установки),
Android сам изолирует их `SharedPreferences` — `device_token` основного
приложения и `device_token` X5 физически не могут перепутаться на
устройстве, даже если оба приложения стоят одновременно.

Тарифы и цены X5 показываются только в боте (раздел «🎯 X5») — само
приложение, как и основное, никогда не видит список тарифов и цену
напрямую, только статус «активна / не активна» через `GateActivity`.
